...
void knock_data_notify(void) {
	wake_up_all(&(knock_data.wait_data_complete_queue_head));
}
...
void update_knock_data(u8 *buf, int size, int frame_id) {
	...
	LOG_INFO("%s: %s frame id %d, size is %d\n", KNOCK_TAG, __func__, frame_id, size);
}
...
void register_frame_count_change_listener(void *listener)
{
	...
	LOG_INFO("%s: %s register need frame count change listener\n", KNOCK_TAG, __func__);
}
...
static ssize_t knock_data_write(struct file *file, const char __user *buf, size_t count, loff_t *pos)
{
	...
	if (result) {
		frame_count = 0;
		LOG_ERROR("%s: %s copy from user failed!\n", KNOCK_TAG, __func__);
		goto end_set_value;
	}
	temp_buf[size] = '\0';
...
static long knock_data_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	if (!cmd) { /* cmd = 0: notify data has update */
		knock_data.has_data_update = true;
		knock_data_notify();
		/* LOG_INFO("%s: %s notify read data from ioctl!\n", KNOCK_TAG, __func__); */
	}
	return 0;
}
...
static unsigned int knock_data_poll(struct file *file, poll_table *wait)
{
	...
	if (knock_data.has_data_update) {
		...
		/* LOG_INFO("%s: %s has knock data need read!\n", KNOCK_TAG, __func__); */
	} else {
		/* LOG_ERROR("%s: %s has empty knock data need read!\n", KNOCK_TAG, __func__); */
	}
	/* LOG_INFO("%s: %s mask is %d!\n", KNOCK_TAG, __func__, mask); */
	return mask;
}
...
static int knock_data_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (remap_pfn_range(vma, start, page, size, PAGE_SHARED))
		...
		LOG_INFO("%s: %s remap_pfn_range %u, size:%ld, success\n", KNOCK_TAG, __func__, (unsigned int)page, size);
...
int knock_node_init(void)
{
	...
	if (!knock_data.raw_data) {
		knock_data.raw_data = kzalloc_retry(KNOCK_RAW_DATA_SIZE, 3);
		if (!knock_data.raw_data) {
			result = -1;
			LOG_ERROR("%s: %s: alloc memory for mmap failed!\n", KNOCK_TAG, __func__);
			goto error_alloc_memory;
		}
		knock_data.phy_base = virt_to_phys(knock_data.raw_data);
		LOG_INFO("%s: %s: raw data addr:%lld, phy addr:%lld\n", KNOCK_TAG, __func__,
	...
	if (result) {
		LOG_INFO("%s: %s: register %s node failed\n", KNOCK_TAG, __func__, KNOCK_NODE_NAME);
		goto error_register_node;
	}
	LOG_INFO("%s: %s: complete!\n", KNOCK_TAG, __func__);
	return 0;

error_register_node:
	if (knock_data.raw_data) {
		kzalloc_free(knock_data.raw_data);
