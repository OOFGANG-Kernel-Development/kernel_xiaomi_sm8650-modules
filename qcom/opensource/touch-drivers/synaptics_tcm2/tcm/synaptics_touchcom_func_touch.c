 ...
 * The default size of data is defined in 16-192 bits natively.
 *
 * @param
 *    [ in] report:       touch report generated by TouchComm device
 *    [ in] report_size:  size of given report
 ...
 *    on success, 0 or positive value; otherwise, negative value on error.
 */
static int syna_tcm_get_knob_calib_data(const unsigned char *report,
		unsigned int report_size, unsigned int offset,
		unsigned int bits, struct tcm_knob_data_blob *knob_data)
{
	int retval;
	unsigned int idx;
	unsigned int slug;
	...
	for (idx = 0; idx < MAX_NUM_KNOB_OBJECTS; idx++) {
		for (slug = 0; slug < 3; slug++) {
			...
			retval = syna_tcm_get_touch_data(report, report_size,
					offset, 16, &data);
			if (retval < 0) {
				LOGE("Fail to get raw data of slug %d\n", slug);
				return retval;
			}

			knob_data[idx].cal_slug_raw[slug] = (unsigned short)data;
			bits -= 16;
			offset += 16;

			LOGD("Knob calib data: idx:%d, slug:%d raw:%d\n",
				idx, slug, knob_data[idx].cal_slug_raw[slug]);
		}
		...
		for (slug = 0; slug < 3; slug++) {
			...
			retval = syna_tcm_get_touch_data(report, report_size,
					offset, 16, &data);
			if (retval < 0) {
				LOGE("Fail to get delta data of slug %d\n", slug);
				return retval;
			}

			knob_data[idx].cal_slug_delta[slug] = (unsigned short)data;
			bits -= 16;
			offset += 16;

			LOGD("Knob calib data: idx:%d, slug:%d delta:%d\n",
				idx, slug, knob_data[idx].cal_slug_delta[slug]);
		}
	}
 ...
 * The default size of data is defined in 16-48 bits natively.
 *
 * @param
 *    [ in] report:       touch report generated by TouchComm device
 *    [ in] report_size:  size of given report
		...
		unsigned int bits, struct tcm_knob_data_blob *knob_data)
{
	...
	for (idx = 0; idx < MAX_NUM_KNOB_OBJECTS; idx++) {
		...
		LOGD("Knob info: idx:%d, angle:%d\n",
			idx, knob_data[idx].angle);
		...
		LOGD("Knob info: idx:%d, click:%d\n",
			idx, knob_data[idx].click);
		...
		LOGD("Knob info: idx:%d, grasp:%d\n",
			idx, knob_data[idx].grasp);
	}
		...
		struct tcm_touch_data_blob *touch_data)
{
	...
	while (idx < config_size) {
		...
		case TOUCH_REPORT_KNOB_CALIB:
			bits = config_data[idx++];
			retval = syna_tcm_get_knob_calib_data(report,
					report_size,
					offset, bits,
					touch_data->knob);
			if (retval < 0) {
				LOGE("Fail to get knob calibration data\n");
				return retval;
			}
			offset += bits;
			break;
	...
	while (idx < size) {
		...
		case TOUCH_REPORT_PAD_TO_NEXT_BYTE:
			if (count_bits_in_loop)
				bits_in_loop = syna_pal_ceil_div(bits_in_loop, 8) * 8;
			if (count_bits_tailing)
				bits_tailing = syna_pal_ceil_div(bits_tailing, 8) * 8;
			break;
		default:
