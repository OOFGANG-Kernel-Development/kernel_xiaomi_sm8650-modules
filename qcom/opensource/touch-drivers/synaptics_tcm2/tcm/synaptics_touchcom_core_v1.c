		...
		unsigned char *status_report_code)
{
	...
	if (tcm_msg->payload_length == 0 && tcm_msg->status_report_code != STATUS_CONTINUED_READ)
		goto do_dispatch;
	...
	/* avoid read wrong length */
	if (tcm_msg->status_report_code == STATUS_CONTINUED_READ) {
		LOGE("report code is not correct,read the next data according to the maximum length:%d\n", tcm_read_max_length);
	...
	/* get max length */
	if(tcm_read_max_length < tcm_msg->payload_length && tcm_msg->status_report_code != STATUS_CONTINUED_READ) {
		tcm_read_max_length = tcm_msg->payload_length;
	}
