...
#define FW_IMAGE_NAME "synaptics_spi_bixi.img"
#endif
...
static int syna_dev_enter_lowpwr_sensing(struct syna_tcm *tcm, unsigned char gesture);
		...
		if (doreflash == 1) {
			...
			if (firmware_flashing == 0) {
				...
				if (inSuspendMode == 1) {
					...
					tcm->palm_sensor_status = 0;
					update_palm_sensor_value(0);

					/* clear all input events  */
					syna_dev_free_input_events(tcm);
#ifdef POWER_ALIVE_AT_SUSPEND
					retval = syna_dev_enter_lowpwr_sensing(tcm, (xiaomi_get_gesture_type(PRI_TOUCH_ID)));
					if (retval < 0) {
						LOGE("Fail to enter suspended power mode\n");
						goto exit;
					}
					if (xiaomi_get_gesture_type(PRI_TOUCH_ID))
						...
						tcm->pwr_state = PWR_OFF;
#endif
					irq_disabled = (!xiaomi_get_gesture_type(PRI_TOUCH_ID));
#ifdef CONFIG_TOUCH_FACTORY_BUILD
					irq_disabled = true;
#endif
				}
 ...
 * 	  [ in] gesture: xiaomi gesture mode
 *
 * @return
 *    on success, 0; otherwise, negative value on error.
 */
static int syna_dev_enable_lowpwr_gesture(struct syna_tcm *tcm, int en, unsigned char gesture)
{
	int retval = 0;
	...
	if (en == 3)
	{
		...
		retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
				...
				RESP_IN_ATTN);
		if (retval < 0) {
			LOGE("Fail to send wakeup gesture mode command en:%d\n",en);
			return retval;
		}
		return retval;
	}
...
static void syna_dev_report_input_events(struct syna_tcm *tcm)
{
	...
	int super_resolution_factor = FW_SUPER_RESOLUTION_FACTOR / tcm->current_super_resolution;
	...
	if (input_dev == NULL)
		return;
	if(max_objects > SYNA_OBJS_CNT_MAX) {
		LOGE("object cnt[%d] is larger than setting[%d]\n",max_objects,SYNA_OBJS_CNT_MAX);
		return;
	}
	syna_pal_mutex_lock(&tcm->tp_event_mutex);
	...
	if ((tcm->pwr_state == LOW_PWR) && tcm->irq_wake) {
		if (touch_data->gesture_id == GESTURE_ID_DOUBLE_TAP && syna_get_wakeup_enable()) {
	...
	if (tcm->pwr_state == LOW_PWR || tcm->pwr_state == PWR_OFF)
		goto exit;
	...
	for (idx = 0; idx < max_objects; idx++) {
		...
		case LIFT:
			...
			break;
		case PALM:
			if (tcm->palm_sensor_enable && !tcm->palm_sensor_status) {
				update_palm_sensor_value(1);
				tcm->palm_sensor_status = 1;
				LOGI("update palm sensor value 1");
			}
			break;
		case FINGER:
		case GLOVED_OBJECT:
...
#endif
			x = (int)((int)(x / super_resolution_factor) * super_resolution_factor);
			y = (int)((int)(y / super_resolution_factor) * super_resolution_factor);
#ifdef TYPE_B_PROTOCOL
			input_mt_slot(input_dev, idx);
			input_mt_report_slot_state(input_dev,
					MT_TOOL_FINGER, 1);
			...
			input_mt_sync(input_dev);
#endif
			LOGCOORD("Super score :%d Finger %d: x = %d, y = %d\n", tcm->current_super_resolution, idx, x, y);
			LOGD("Super score :%d Finger %d: x = %d, y = %d\n", tcm->current_super_resolution, idx, x, y);
			touch_count++;
		...
		if ((status == PALM) || (status == FINGER) || (status == GLOVED_OBJECT)) {
	...
	if (prev_obj_present != curr_obj_present){
		...
		for(i = 0; i<max_objects; i++) {
			if(xorVal&(1<<i)){
				LOGI("Finger[%2d]:%d  Gesture Event:%02x\n",
					i,tcm->prev_obj_status[i],touch_data->gesture_id);
			}
		}
...
static irqreturn_t syna_dev_isr(int irq, void *data)
{
	...
	if(tcm->tcm_data_dump.data_dump_report_type != 0 && code == tcm->tcm_data_dump.data_dump_report_type)
	{
		LOGI("copy %x to dump matrix!\n",code);
		syna_tcm_buf_lock(&tcm->tcm_dev->external_buf);
		retval = syna_tcm_buf_copy(&tcm->tcm_data_dump.data_dump_report,&tcm->tcm_dev->external_buf);
		if (retval < 0) {
			LOGI("copy %x to dump matrix failed!\n",code);
		}else{
			complete(&tcm->tcm_data_dump.data_dump_completion);
		}
...
void syna_dev_reflash_startup(struct syna_tcm *tcm, bool force_reflash)
{
	...
	retval = request_firmware(&fw_entry,
			...
			&tcm->pdev->dev);
	if (retval < 0) {
		LOGE("[DIS-TF-TOUCH] Fail to request %s\n", FW_IMAGE_NAME);
		return;
	}
	...
	if (retval < 0) {
		LOGE("[DIS-TF-TOUCH] Fail to do reflash\n");
		goto exit;
	}
 ...
 *    on success, 0; otherwise, negative value on error.
 */
static int syna_dev_enter_normal_sensing(struct syna_tcm *tcm, unsigned char gesture)
{
	int retval = 0;
	...
	retval = syna_dev_enable_lowpwr_gesture(tcm, 0, gesture);
	if (retval < 0) {
...
static int syna_dev_enter_lowpwr_sensing(struct syna_tcm *tcm, unsigned char gesture)
{
	...
	if (gesture) {
		retval = syna_tcm_set_gesture_type(tcm, gesture);
		if (retval < 0) {
			LOGE("set gesture type error\n");
			return retval;
		}
		retval = syna_dev_enable_lowpwr_gesture(tcm, 1, gesture);
		if (retval < 0) {
			LOGE("Fail to disable low power gesture mode\n");
			return retval;
		}
	} else {
		retval = syna_dev_enable_lowpwr_gesture(tcm, 3, gesture);
		if (retval < 0) {
			LOGE("Fail to disable low power gesture mode\n");
			return retval;
		}
 ...
 *    on success, 0; otherwise, negative value on error.
 */
static int syna_dev_resume(struct device *dev, unsigned char gesture_type)
{
	int retval;
	...
	int i = 0, retry = 3, failCount = 0;
	pct = &tcm->hw_if->bdata_pct;
	/* exit directly if device isn't in suspend state */
#ifdef TOUCH_SYNA_BOOTLOADER_RECOVERY
	inSuspendMode = 0;
#endif
	if (tcm->pwr_state == PWR_ON && IS_APP_FW_MODE(tcm->tcm_dev->dev_mode))
		return 0;

	LOGI("Prepare to resume device\n");

	retval = get_capfold_information();
	if (retval < 0)
		LOGE("get capfold information failed, retval:%d\n", retval);
	...
	for (i = 0; i < retry; i++) {
		...
		retval = syna_dev_enter_normal_sensing(tcm, gesture_type);
		if (retval < 0) {
			failCount++;
			LOGE("Fail to enter normal power mode try again %d\n",failCount);
			if(failCount == retry){
				LOGE("Fail to enter normal power mode %d times,try to hw reset\n",failCount);
			...
			LOGI("enter normal power mode success,failCount = %d\n",failCount);
			break;
		}
	}
#endif
 ...
 *    on success, 0; otherwise, negative value on error.
 */
static int syna_dev_suspend(struct device *dev, unsigned char gesture_type)
{
	int retval;
	struct syna_tcm *tcm = dev_get_drvdata(dev);
	struct syna_hw_interface *hw_if = tcm->hw_if;
	...
	retval = syna_dev_enter_lowpwr_sensing(tcm, gesture_type);
	if (retval < 0) {
		LOGE("Fail to enter suspended power mode\n");
		return retval;
	}
		...
		unsigned long action, void *data)
{
	...
	if (action == disp_early_event_blank &&
		...
		if (transition == disp_blank_powerdown) {
			retval = syna_dev_suspend(&tcm->pdev->dev, xiaomi_get_gesture_type(PRI_TOUCH_ID));
			tcm->fb_ready = 0;
		} else if (transition == disp_blank_unblank) {
#ifndef RESUME_EARLY_UNBLANK
			retval = syna_dev_resume(&tcm->pdev->dev, xiaomi_get_gesture_type(PRI_TOUCH_ID));
			tcm->fb_ready++;
#endif
...
/* initialize the gpio pins */
	...
	if (retval < 0) {
		LOGE("[DIS-TF-TOUCH] Fail to config gpio\n");
		return retval;
	}
...
request_irq:
	...
	syna_tcm_change_resp_read(tcm->tcm_dev,RESP_IN_ATTN);
	xiaomi_touch_probe(tcm);
...
#else
		LOGE("[DIS-TF-TOUCH] Fail to connect to the device\n");
