...
{
	struct ais_ife_event_data *evt_data = NULL;
	uint8_t vfe_idx = 0;

	if (old == NULL)
		return;

	evt_data = (struct ais_ife_event_data *)old->u.data;
	if ((old->id != V4L_EVENT_ID_AIS_IFE) || (old->type != V4L_EVENT_TYPE_AIS_IFE))
		return;

	if ((evt_data == NULL) || (evt_data->msg.type != AIS_IFE_MSG_FRAME_DONE))
		return;

	vfe_idx = evt_data->msg.idx;
		...
		struct cam_isp_query_cap_cmd *query_isp)
{
	...
	for (i = 0; i < query_isp->num_dev; i++) {
		...
		query_isp->dev_caps[i].hw_version.minor = 8;
	...
	struct cam_smmu_pf_info *pf_info)
{
	...
	p_ife_dev = (struct ais_ife_dev *)pf_info->token;

	CAM_ERR(CAM_ISP, "IFE%d Pagefault at iova 0x%x %u",
		p_ife_dev->hw_idx, pf_info->iova, pf_info->domain->type);
}
	...
	struct device *master_dev, void *data)
{
	...
	CAM_INFO(CAM_ISP, "IFE%d probe", p_ife_dev->hw_idx);
	...
	ais_ife_csid_set_vfe_hw(p_ife_dev->p_csid_drv, p_ife_dev->p_vfe_drv);
...
const static struct component_ops ais_isp_dev_component_ops = {
...
{
	component_del(&pdev->dev, &ais_isp_dev_component_ops);
	return 0;
}
...
static int ais_ife_dev_probe(struct platform_device *pdev)
{
	...
	rc = component_add(&pdev->dev, &ais_isp_dev_component_ops);
	if (rc)
