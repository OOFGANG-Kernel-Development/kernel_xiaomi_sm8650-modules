...
static int ais_vfe_create_debugfs_dir(void)
{
	int rc = 0;
	...
	if (!debugfs_root) {
		rc = cam_debugfs_create_subdir("ais_vfe", &dbgfileptr);
		if (rc) {
...
static int ais_vfe_component_bind(struct device *dev,
	struct device *master_dev, void *data)
{
	...
	core_info->total_size = vfe_hw->soc_info.reg_map[0].size;
	rc = ais_vfe_create_debugfs_dir();
	if (rc) {
...
static void ais_vfe_component_unbind(struct device *dev,
	struct device *master_dev, void *data)
{
	struct cam_hw_info                *vfe_hw = NULL;
	struct cam_hw_intf                *vfe_hw_intf = NULL;
...
const static struct component_ops ais_vfe_component_ops = {
	.bind = ais_vfe_component_bind,
	.unbind = ais_vfe_component_unbind,
};
...
int ais_vfe_probe(struct platform_device *pdev)
{
	...
	rc = component_add(&pdev->dev, &ais_vfe_component_ops);
	if (rc)
...
{
	component_del(&pdev->dev, &ais_vfe_component_ops);
	return 0;
}
