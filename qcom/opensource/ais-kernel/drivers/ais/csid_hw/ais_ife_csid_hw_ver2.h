...
#ifndef _AIS_IFE_CSID_HW_VER2_H_
#define _AIS_IFE_CSID_HW_VER2_H_
...
struct cam_ife_csid_ver2_path_cfg {
	...
	struct cam_ife_csid_path_format      path_format[AIS_ISP_VC_DT_CFG];
	//struct cam_csid_secondary_evt_config sec_evt_config;
	uint32_t                             cid;
	uint32_t                             in_format[AIS_ISP_VC_DT_CFG];
	uint32_t                             out_format;
...
struct cam_ife_csid_ver2_common_reg_info {
	...
	uint32_t drv_rup_en_val_map[AIS_IFE_PIX_PATH_RES_MAX];
	uint32_t drv_path_idle_en_val_map[AIS_ISP_MAX_PATHS];
...
/*
 * struct ais_ife_csid_csi2_rx_reg_info: Structure to hold Rx reg offset
...
struct cam_ife_csid_ver2_reg_info {
	struct ais_irq_controller_reg_info               *top_irq_reg_info;
	struct ais_irq_controller_reg_info               *rx_irq_reg_info;
	struct ais_irq_controller_reg_info               *buf_done_irq_reg_info;
	struct ais_irq_controller_reg_info               *path_irq_reg_info[
		AIS_IFE_PIX_PATH_RES_MAX];
	...
	const uint32_t                                    csid_cust_node_map[
			AIS_IFE_CSID_HW_NUM_MAX];
	const int                                         input_core_sel[
			AIS_IFE_CSID_HW_NUM_MAX][CAM_IFE_CSID_INPUT_CORE_SEL_MAX];
...
/*struct cam_ife_csid_ver2_hw {
	...
	void                                  *path_irq_controller[AIS_IFE_PIX_PATH_RES_MAX];
...
/*struct cam_ife_csid_ver2_mini_dump_data {
	struct cam_ife_csid_ver2_res_mini_dump  res[AIS_IFE_PIX_PATH_RES_MAX];
	struct cam_ife_csid_hw_flags            flags;
	struct cam_ife_csid_rx_cfg              rx_cfg;
	struct cam_ife_csid_cid_data            cid_data[CAM_IFE_CSID_CID_MAX];
	uint64_t                                clk_rate;
