...
int32_t cam_aperture_apply_request(struct cam_req_mgr_apply_request *apply)
{
	...
	if (a_ctrl->skip_frame_queue[apply->request_id % MAX_PER_FRAME_ARRAY].req_id == apply->request_id &&
		a_ctrl->skip_frame_queue[apply->request_id % MAX_PER_FRAME_ARRAY].skip_num >= 1) {
		rc = -EAGAIN;
		CAM_INFO(CAM_APERTURE, "[CRM-APERTURE] ApeSkipFrame at request:%llu", apply->request_id);
	}

	if (a_ctrl->skip_frame_queue[apply->request_id % MAX_PER_FRAME_ARRAY].req_id == apply->request_id &&
		a_ctrl->skip_frame_queue[apply->request_id % MAX_PER_FRAME_ARRAY].skip_num > 0) {
		CAM_INFO(CAM_APERTURE, "[CRM-APERTURE] ApeSkipFrame number from %llu to %llu at request:%llu",
				 a_ctrl->skip_frame_queue[apply->request_id % MAX_PER_FRAME_ARRAY].skip_num,
				 a_ctrl->skip_frame_queue[apply->request_id % MAX_PER_FRAME_ARRAY].skip_num - 1,
				 apply->request_id);
		a_ctrl->skip_frame_queue[apply->request_id % MAX_PER_FRAME_ARRAY].skip_num--;
		if (0 == a_ctrl->skip_frame_queue[apply->request_id % MAX_PER_FRAME_ARRAY].skip_num)
		{
			a_ctrl->skip_frame_queue[apply->request_id % MAX_PER_FRAME_ARRAY].req_id = 0;
		}
	}
	...
	void *arg)
{
	...
	case CAM_APERTURE_PACKET_SWITCH:
		...
		if (0 < skip_frame_msg->skip_num) {
			a_ctrl->skip_frame_queue[
				skip_frame_msg->req_id % MAX_PER_FRAME_ARRAY].req_id =
					skip_frame_msg->req_id;

			if(skip_frame_msg->trigger_eof == true)
			{
				a_ctrl->trigger_eof = true;
				a_ctrl->skip_frame_queue[
			...
			{
				a_ctrl->trigger_eof = false;
				a_ctrl->skip_frame_queue[
					skip_frame_msg->req_id % MAX_PER_FRAME_ARRAY].skip_num =
						skip_frame_msg->skip_num;
			}

			CAM_INFO(CAM_APERTURE, "[ApeSkipFrame] skip reqId: %llu, skip num: %llu queue index:%d  trigger_eof %d",
					a_ctrl->skip_frame_queue[(skip_frame_msg->req_id) % MAX_PER_FRAME_ARRAY].req_id,
					a_ctrl->skip_frame_queue[(skip_frame_msg->req_id) % MAX_PER_FRAME_ARRAY].skip_num,
		...
		rc = cam_aperture_update_req_mgr(a_ctrl, csl_packet);
		if (rc) {
			CAM_ERR(CAM_APERTURE,
	...
	case CAM_FRAME_SKIP_OPCODE: {
		if (a_ctrl->cam_act_state < CAM_APERTURE_CONFIG) {
			CAM_ERR(CAM_APERTURE,
				"[ApeSkipFrame] Recvied skip frame packet in invalid state: %d",
				a_ctrl->cam_act_state);
			rc = -EINVAL;
			goto end;
		}
		...
		CAM_INFO(CAM_APERTURE, "[ApeSkipFrame] num_cmd_buf %d",
			csl_packet->num_cmd_buf);

		if (0 == csl_packet->num_cmd_buf) {
			CAM_ERR(CAM_APERTURE,
				"[ApeSkipFrame] Recvied skip frame packet is empty.");
			rc = -EINVAL;
			goto end;
		}
		...
		CAM_INFO(CAM_APERTURE, "[ApeSkipFrame] total_cmd_buf_in_bytes %d",
			total_cmd_buf_in_bytes);

		if (sizeof(struct skip_frame) > total_cmd_buf_in_bytes) {
			CAM_ERR(CAM_APERTURE,
				"[ApeSkipFrame] Insufficient number of packet bytes received: %d.",
				total_cmd_buf_in_bytes);
			rc = -EINVAL;
			goto end;
		}
		...
		if ((len_of_buff < sizeof(struct skip_frame)) ||
			...
			CAM_ERR(CAM_APERTURE,
				"[ApeSkipFrame] Invalid length for skip frame packet");
			return -EINVAL;
		}
	...
	void *arg)
{
	...
	case CAM_RELEASE_DEV: {
		...
		memset(a_ctrl->skip_frame_queue,
			0,
