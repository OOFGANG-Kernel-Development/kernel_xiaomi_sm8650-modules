	...
	case CAM_ACTUATOR_PACKET_OPCODE_INIT:
		...
		if (a_ctrl->cam_act_state == CAM_ACTUATOR_ACQUIRE) {
			if (parklens_power_down == true) {//xiaomi add
				...
				if (rc < 0) {
					CAM_ERR(CAM_ACTUATOR,
						"[ParklensLog] Actuator Power up failed");
					goto end;
				}
			}//xiaomi add
...
void cam_actuator_shutdown(struct cam_actuator_ctrl_t *a_ctrl)
{
	...
	if (a_ctrl->cam_act_state >= CAM_ACTUATOR_CONFIG) {
		if(false == is_parklens_power_down(a_ctrl)) {
			...
			if (rc < 0)
				CAM_ERR(CAM_ACTUATOR, "[ParklensLog] Actuator Power down failed");
			CAM_INFO(MI_PARKLENS, "[ParklensLog] parklens isn't powerdown");
		} else {
